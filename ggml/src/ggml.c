...
#define LUT_ALPHA 1000
static float* global_lut_table = NULL;

// LUT[i,j] = (i*j) / alpha^2 [cite: 218]
void ggml_init_lut_table() {
    if (global_lut_table) return;
    global_lut_table = (float*)malloc((LUT_ALPHA + 1) * (LUT_ALPHA + 1) * sizeof(float));
    for (int i = 0; i <= LUT_ALPHA; i++) {
        for (int j = 0; j <= LUT_ALPHA; j++) {
            global_lut_table[i * (LUT_ALPHA + 1) + j] = (float)(i * j) / (float)(LUT_ALPHA * LUT_ALPHA);
        }
    }
}

void ggml_compute_forward_lut_attn(
        struct ggml_compute_params * params,
        struct ggml_tensor * dst,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1) {
    // src0 shape: [D, 1, 1, 1]
    // src1 shape (logical): [L, D] -> Physical SoA: [D, L]
    const int64_t D = src0->ne[0]; // Head Dimension (feature dim)
    const int64_t L = src1->ne[1]; // Sequence Length (number of keys)


    const uint16_t* q_indices = (const uint16_t*) src0->data; 
    const uint8_t* q_signs   = (const uint8_t*)  ((char*)src0->data + D * sizeof(uint16_t));
    const uint16_t* k_indices_base = (const uint16_t*) src1->data;
    const uint8_t* k_signs_base   = (const uint8_t*)  ((char*)src1->data + D * L * sizeof(uint16_t));

    float* dst_scores = (float*) dst->data;
    const int ith = params->ith; // 当前线程 ID
    const int nth = params->nth; // 总线程数

    // 计算当前线程处理的 L 范围
    const int64_t l_per_thread = (L + nth - 1) / nth;
    const int64_t l_start = ith * l_per_thread;
    const int64_t l_end   = (l_start + l_per_thread > L) ? L : l_start + l_per_thread;

    if (l_start >= l_end) return;

    memset(dst_scores + l_start, 0, (l_end - l_start) * sizeof(float));
    for (int64_t d = 0; d < D; ++d) {
        uint16_t q_idx = q_indices[d];
        const float* active_lut_row = GGML_LUT_TABLE + (q_idx * LUT_STRIDE);
        
        float q_sign_val = (q_signs[d] == 0) ? 1.0f : -1.0f;
        __m512 v_q_sign = _mm512_set1_ps(q_sign_val);
        if (d + 1 < D) {
            uint16_t next_q_idx = q_indices[d + 1];
            const float* next_lut_row = GGML_LUT_TABLE + (next_q_idx * LUT_STRIDE);
            // 预取
            _mm_prefetch((const char*)next_lut_row, _MM_HINT_T0);
            _mm_prefetch((const char*)(next_lut_row + 16), _MM_HINT_T0); // +64 bytes
        }

        
        // Dim d 的偏移是 d * L
        const uint16_t* k_idx_ptr = k_indices_base + (d * L);
        const uint8_t* k_sgn_ptr = k_signs_base   + (d * L);
        // Inner loop over Sequence Length (Key Tokens)
        int64_t j = l_start;
        for (; j <= l_end - 16; j += 16) {
            // A. Load Key Indices (SoA Contiguous) 
            // "SoA layout provides the entire slice K_{j:j+W,d} via one aligned load"
            __m256i v_k_idx_16 = _mm256_loadu_si256((const __m256i*)(k_idx_ptr + j));
            __m512i v_k_idx_32 = _mm512_cvtepu16_epi32(v_k_idx_16);
            __m512 v_lut_val = _mm512_i32gather_ps(v_k_idx_32, active_lut_row,4);
            __m128i v_k_sgn_8 = _mm_loadu_si128((const __m128i*)(k_sgn_ptr + j)); // Load 16 bytes (use lower 128 of 256/512 reg)
            __m512i v_k_sgn_32 = _mm512_cvtepu8_epi32(v_k_sgn_8); // Extend to 32-bit integer
            __mmask16 neg_mask = _mm512_test_epi32_mask(v_k_sgn_32, _mm512_set1_epi32(1));
           
            __m512 v_k_sign_val = _mm512_mask_blend_ps(neg_mask, _mm512_set1_ps(1.0f), _mm512_set1_ps(-1.0f));

            __m512 v_term = _mm512_mul_ps(v_lut_val, v_q_sign);
            v_term = _mm512_mul_ps(v_term, v_k_sign_val);

            // Load current partial sums from dst
            __m512 v_acc = _mm512_loadu_ps(dst_scores + j);
            v_acc = _mm512_add_ps(v_acc, v_term);
            _mm512_storeu_ps(dst_scores + j, v_acc);
        }

        for (; j < l_end; ++j) {
            uint16_t k_idx = k_idx_ptr[j];
            uint8_t k_sgn = k_sgn_ptr[j];
            float k_sign_val = (k_sgn == 0) ? 1.0f : -1.0f;
            
            // LUT lookup
            float val = active_lut_row[k_idx];
            
            // Accumulate
            dst_scores[j] += val * q_sign_val * k_sign_val;
        }
    }
}
